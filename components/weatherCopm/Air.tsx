...
const Air: FC<AirProps> = ({}) => {
  ...
  const { data, isPending } = useQuery({
    queryKey: ["air"],
    queryFn: () =>
      axios.get(`/api/weather/air_pollution?lat=${lat}&lon=${lon}`),
  })
  return (
    <>
      {isPending ? (
        ...
        <section className="space-y-12">
          ...
          <Progress
            className="progress"
            value={data?.data.list[0].main.aqi * 10}
          ...
          <p>
            {data?.data.list[0].main.aqi < 50
              ? "Air quality is good."
              : data?.data.list[0].main.aqi < 100
              ? "Air quality is moderate."
              : data?.data.list[0].main.aqi < 150
              ? "Air quality is unhealthy for sensitive groups."
              : data?.data.list[0].main.aqi < 200
              ? "Air quality is unhealthy."
              : data?.data.list[0].main.aqi < 300
